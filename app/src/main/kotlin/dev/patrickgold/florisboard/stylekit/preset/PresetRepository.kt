...
class PresetRepository(context: Context) {
    ...
    /**
     * Serializes [presets] to a portable JSON document (see [PresetExport]),
     * for the "Export presets" flow in the Presets screen. Built-in presets
     * are included too (harmless to export — [importFromJson] always inserts
     * them back in as user, non-built-in presets), so the user gets one file
     * with everything currently in their library.
     */
    fun exportToJson(presets: List<PresetEntity>): String {
        val export = presets.map {
            PresetExport(name = it.name, description = it.description, mapping = DefaultPresets.decode(it.mappingJson))
        }
        return DefaultPresets.encodeExport(export)
    }

    /**
     * Parses a JSON document previously produced by [exportToJson] (or hand-
     * written in the same shape) and inserts each entry as a new, non-built-in
     * preset. Name collisions with existing presets are resolved by
     * appending " (imported)" (and a counter if that's also taken) rather
     * than silently overwriting anything.
     *
     * Returns the number of presets successfully imported. Never throws —
     * a malformed file or an individual bad entry is skipped rather than
     * aborting the whole import.
     */
    suspend fun importFromJson(json: String): Int {
        val entries = DefaultPresets.decodeExport(json)
        if (entries.isEmpty()) return 0
        val existingNames = getAll().map { it.name }.toMutableSet()
        var imported = 0
        for (entry in entries) {
            if (entry.mapping.isEmpty()) continue
            var name = entry.name.ifBlank { "Imported preset" }
            ...
            val id = insert(name, entry.mapping, entry.description)
            if (id >= 0) {
